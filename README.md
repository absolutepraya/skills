...
```bash
npx skills add absolutepraya/skills --skill <skill-name>
```
...
|---|---|---|
| [`scele`](./skills/scele) | CLI for Fasilkom UI's SCELE Moodle LMS — fetch deadlines, browse course materials, submit assignments, read forum threads, bind PDFs, etc. Bash 4+, requires SCELE creds. | `npx skills add absolutepraya/skills --skill scele` |
| [`scele-digest`](./skills/scele-digest) | Companion to `scele`. Daily digest posted to Discord — SCELE deadlines + announcements + class-chat Telegram summary + Todoist sync. Python, supports webhook or bot delivery. | `npx skills add absolutepraya/skills --skill scele-digest` |
| [`architecture-image`](./skills/architecture-image) | Create crisp, editable SVG architecture, integration, workflow, and system diagrams, then render PNG assets with Node or Python. | `npx skills add absolutepraya/skills --skill architecture-image` |
